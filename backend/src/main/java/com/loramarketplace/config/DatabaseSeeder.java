...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        List<Product> candidates = buildSeedProducts();
        ...
        if (!toSave.isEmpty()) {
            productRepository.saveAllAndFlush(toSave);
            System.out.println("Seeded database with " + toSave.size() + " LoRA(s).");
        } else {
            System.out.println("Database already seeded — no new LoRAs to insert.");
        }
    }
